...
app.get("/", (req, res) => {
  res.status(200).json({
    success: true,
    message: "Welcome to Job Board Server....",
  });
});
...
app.listen(PORT, () => {
  ConnectDB()
    .then(() => {
      console.log(
        `Job-Board Server is running on PORT: http://localhost:${PORT}\nDatabase Connected Successfully.....`
      );
    })
    .catch((error) => {
      console.log("Job-Board Server is Running but Database connection failed !!!");
      console.log(error);
    });
});
